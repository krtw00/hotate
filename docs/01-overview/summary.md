...
ai_summary: "Hotateの1ページ概要 — ブラウザベースSSHクライアント（目的・主要機能・対象ユーザー・構成図）"
---
...
本ドキュメントは、Hotateプロジェクト全体を1枚で把握するための概要を記載する。

---

## 一言で言うと

Hotateは、モバイル端末からSSHサーバーに接続するためのIME対応ブラウザターミナルを提供するWebアプリケーションである。
...
| 解決アプローチ | ブラウザ上でxterm.jsターミナルとIME対応入力バーを組み合わせ、モバイルから快適にSSH操作できる環境を構築する |
...
| tmux タブ管理 | tmux attach を自動検出し、ウィンドウ一覧をタブバーで表示。タップで切り替え、デタッチボタン付き |
