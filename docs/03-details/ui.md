...
| tmuxタブバー | タブ列 | `#tmux-tabs` — tmux attach検出時にヘッダーとターミナル間に表示。ウィンドウタブ（名前+インデックス）を並べ、アクティブウィンドウをハイライト。右端にデタッチボタン（赤）。デタッチ検出時に自動非表示。3秒ポーリングでウィンドウ一覧を更新 |
...
| tmuxウィンドウタブをタップ | PTY経由でCtrl+B → ウィンドウインデックスを送信してウィンドウ切替 |
| デタッチボタンをタップ | PTY経由でCtrl+B d を送信してtmuxデタッチ |
